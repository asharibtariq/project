<table class="table table-hover">
    ...
    <tr>
        ...
        <th> FY </th>
        <th> Date </th>
        <th> Amount Released (<small class="text-muted">PKR</small>) </th>
        <th> Foreign Aid </th>
        <th> Action </th>
    </tr>
    </thead>
    ...
    @if(is_array($result) && count($result) > 0)
        @foreach ($result as $r)
            ...
            $alloc_amount = $alloc_amount + $r->rel_amount;
            $foreign_alloc_amount = $foreign_alloc_amount + $r->foreign_rel_amount;
            ?>
            ...
            <tr role="row">
                ...
                <td> {{$r->rel_amount}} </td>
                <td> {{$r->foreign_rel_amount}} (<small class="text-muted">{{$r->currency}}</small>) </td>
                <td>
                    <div class="btn-group">
                        <a onClick="return confirm('Are you sure you want to update?');" title="Edit" href="{{url('edit_release', $r->id)}}" class="btn btn-info" id="btn-view"><i class="fa fa-edit"></i> </a>
